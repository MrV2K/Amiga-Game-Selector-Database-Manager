...
Structure UM_Data
  ...
  UM_JIT.b
  UM_Update.b
  UM_Image_Cover_CRC.s
  UM_Screen_Cover_CRC.s
  UM_Title_Cover_CRC.s
EndStructure
...
Procedure Create_Menus()
  ...
  MenuItem(#MenuItem_27,  "Edit Entry",ImageID(#TEXT_IMAGE))
  MenuBar()
  ...
  MenuBar() 
  MenuItem(#MenuItem_18b, "Create Game List",ImageID(#CREATE_ICON))
  ...
  MenuItem(#MenuItem_18f, "Process Tosec",ImageID(#CREATE_ICON))
  MenuBar() 
...
Procedure Load_DB()
  ...
  If ListSize(Fix_List())>0
    ForEach Fix_List()
      ...
      UM_Database()\UM_Image_Cover_CRC=Fix_List()\F_Image_Cover_CRC
      UM_Database()\UM_Screen_Cover_CRC=Fix_List()\F_Screen_Cover_CRC
      UM_Database()\UM_Title_Cover_CRC=Fix_List()\F_Title_Cover_CRC
    Next
  EndIf
...
Procedure Save_DB()
  ...
  ForEach UM_Database()
    ...
    Fix_List()\F_Image_Cover_CRC=UM_Database()\UM_Image_Cover_CRC
    Fix_List()\F_Screen_Cover_CRC=UM_Database()\UM_Screen_Cover_CRC
    Fix_List()\F_Title_Cover_CRC=UM_Database()\UM_Title_Cover_CRC
  Next
...
Macro Make_AGS_Run(info_path)
  ...
  If Real=#True
    ...
    Add_Script(" SetEnv XPath "+#DOUBLEQUOTE$+"AGS:"+list_start+current_folder+info_path+#DOUBLEQUOTE$+#LF$)
    Add_Script(" SetEnv XFile "+#DOUBLEQUOTE$+info_path+#DOUBLEQUOTE$+#LF$)
    Add_Script(" SetEnv XFolder "+#DOUBLEQUOTE$+"AGS_Drive:"+UM_Database()\UM_Type+"/"+UM_Database()\UM_Subfolder+"/"+UM_Database()\UM_Folder+"/"+#DOUBLEQUOTE$+#LF$)
  ...
  Add_Script("cd "+#DOUBLEQUOTE$+"AGS_Drive:"+UM_Database()\UM_Type+"/"+UM_Database()\UM_Subfolder+"/"+UM_Database()\UM_Folder+#DOUBLEQUOTE$+#LF$)
  ...
  EndIf
  Add_Script(" SetEnv XFave "+#DOUBLEQUOTE$+#DOUBLEQUOTE$+#LF$)
  Add_Script(" SetEnv XPath "+#DOUBLEQUOTE$+#DOUBLEQUOTE$+#LF$)
  Add_Script(" SetEnv XFile "+#DOUBLEQUOTE$+#DOUBLEQUOTE$+#LF$)
  Add_Script(" SetEnv XFolder "+#DOUBLEQUOTE$+#DOUBLEQUOTE$+#LF$)
  Add_Script(" SetEnv XInfo "+#DOUBLEQUOTE$+#DOUBLEQUOTE$+#LF$)
  Add_Script(" SetEnv XType "+#DOUBLEQUOTE$+#DOUBLEQUOTE$+#LF$)
  Add_Script(" SetEnv XSubfolder "+#DOUBLEQUOTE$+#DOUBLEQUOTE$+#LF$)
...
Procedure Create_AGS_Files()
  ...
  If AGS_Path<>""
    ...
    If MessageRequester("Information","AGS Classic Setup?",#PB_MessageRequester_YesNo|#PB_MessageRequester_Info)=#PB_MessageRequester_Yes
      Real=#True
    ...
    If AGS_Path<>""
      ...
      DeleteDirectory(AGS_Path+"WHD-Data","*.*",#PB_FileSystem_Force)
...
Procedure Settings_Window()
  ...
  If OpenWindow(#PATH_WINDOW, 0, 0, 490, 580, "UltraMiggy Settings", #PB_Window_SystemMenu|#PB_Window_WindowCentered, WindowID(#MAIN_WINDOW))
...
Procedure Edit_Database(number) 
  ...
  Protected Next_Button, Prev_Button, Publisher_Text, WHD_Button, Restricted_Check, Data_Disk, Adult_Check, Pure_Check, Publisher_Title.i
  Protected Info_Container, Filter_Container, Slave_String, CRC_String
  ...
  If OpenWindow(#EDIT_WINDOW, 0, 0, 450, 590, "Edit Database ("+Str(ListIndex(List_Numbers()))+" of "+Str(ListSize(List_Numbers()))+")", #PB_Window_SystemMenu | #PB_Window_WindowCentered, WindowID(#MAIN_WINDOW))
    ...
    Slave_String = StringGadget(#PB_Any, 80, 138, 360, 24, UM_Database()\UM_Slave)
    If UM_Database()\UM_Type="Demo" Or UM_Database()\UM_Beta_Type="Demo"
    ...
    CRC_String = StringGadget(#PB_Any, 350, 202, 90, 24, UM_Database()\UM_CRC32,#PB_String_ReadOnly)    
    ...
    Party_String = StringGadget(#PB_Any,276,368,140,24,UM_Database()\UM_Party)
    TextGadget(#PB_Any, 212, 370, 60, 20, "Rel. Party") 
    ...
    TextGadget(#PB_Any, 108, 500, 50, 20, "HOL New")   
    HOL_String_New = StringGadget(#PB_Any,165,498,175,24,UM_Database()\UM_HOL_Entry_New)   
    TextGadget(#PB_Any, 350, 500, 40, 20, "Lemon #")    
    Lemon_String = StringGadget(#PB_Any,395,498,50,24,UM_Database()\UM_Lemon_Entry)
    ...
    TextGadget(#PB_Any, 212, 530, 60, 20, "WHDLoad") 
    WHD_String = StringGadget(#PB_Any,276,528,120,24,UM_Database()\UM_WHD_Entry)
    WHD_Button = ButtonGadget(#PB_Any,398,528,20,24,"Go")
...
Procedure Get_Image_CRC()
  
  Protected f_cover.s, f_screen.s, f_title.s
  
  ForEach UM_Database()
    f_cover=Game_Data_Path+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Icon,#PB_FileSystem_NoExtension)+"_Cover.png"
    f_screen=Game_Data_Path+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Icon,#PB_FileSystem_NoExtension)+"_Screen.png"
    f_title=Game_Data_Path+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Icon,#PB_FileSystem_NoExtension)+"_Title.png"
    UM_Database()\UM_Image_Cover_CRC=FileFingerprint(f_cover,#PB_Cipher_CRC32)
    UM_Database()\UM_Title_Cover_CRC=FileFingerprint(f_title,#PB_Cipher_CRC32)
    UM_Database()\UM_Screen_Cover_CRC=FileFingerprint(f_screen,#PB_Cipher_CRC32)
  Next
      
EndProcedure

Procedure Scrape_HOL()
  
  Protected HOL_Window, HOL_Gadget, output.s, type, test$
  
  HOL_Window=OpenWindow(#PB_Any,640,480,640,480,"Http",#PB_Window_ScreenCentered|#PB_Window_SystemMenu)
  HOL_Gadget=WebGadget(#PB_Any,0,0,640,480,"http://www.google.com/",#PB_Web_Edge)
  
  ForEach UM_Database()  
    output=""
    If UM_Database()\UM_HOL_Entry<>""   
      SetGadgetText(HOL_Gadget,"http://hol.abime.net/"+UM_Database()\UM_HOL_Entry) 
      Repeat 
        type=EventType()
        WindowEvent()
        If type=#PB_EventType_DownloadEnd
          output.s=GetGadgetText(HOL_Gadget)
          If output<>"about:blank"
            count=CountString(output,"/")
            test$=StringField(output,count+1,"/")
            UM_Database()\UM_HOL_Entry_New=test$
            Break
          EndIf
        EndIf
      ForEver
...
;   ForEach UM_Database()
;     UM_Database()\UM_Title=Trim(UM_Database()\UM_Title)
;     UM_Database()\UM_Short=Trim(UM_Database()\UM_Short)
;     UM_Database()\UM_Publisher=Trim(UM_Database()\UM_Publisher)
;     UM_Database()\UM_Developer=Trim(UM_Database()\UM_Developer)
;     UM_Database()\UM_Party=Trim(UM_Database()\UM_Party)
;     UM_Database()\UM_Subfolder=Trim(UM_Database()\UM_Subfolder)
;     UM_Database()\UM_Year=Trim(UM_Database()\UM_Year)
;     UM_Database()\UM_HOL_Entry=Trim(UM_Database()\UM_HOL_Entry)
;     UM_Database()\UM_HOL_Entry_New=Trim(UM_Database()\UM_HOL_Entry_New)
;     UM_Database()\UM_WHD_Entry=Trim(UM_Database()\UM_WHD_Entry)
;     UM_Database()\UM_Lemon_Entry=Trim(UM_Database()\UM_Lemon_Entry)
;     UM_Database()\UM_Language=Trim(UM_Database()\UM_Language)
;     UM_Database()\UM_LHAFile=Trim(UM_Database()\UM_LHAFile)
;     UM_Database()\UM_Folder=Trim(UM_Database()\UM_Folder)
;     UM_Database()\UM_Disks=Trim(UM_Database()\UM_Disks)
;     UM_Database()\UM_Memory=Trim(UM_Database()\UM_Memory)
;   Next
  
;   path=OpenFileRequester("","","e:",-1)
;   
;   i=ReadFile(#PB_Any,path)
;   
;   Protected NewMap test.s()
  
;   While Not Eof(i)
;     path2=ReadString(i)
;     test(LCase(path2))=path2
;   Wend
;   
;   ForEach UM_Database()
;     If UM_Database()\UM_Type="Game" Or UM_Database()\UM_Beta_Type="Game"
;       If Not FindMapElement(test(),LCase(UM_Database()\UM_WHD_Entry+".html"))
;         If UM_Database()\UM_WHD_Entry<>""
;           Debug UM_Database()\UM_WHD_Entry
;         Else
;           Debug UM_Database()\UM_Title+Tags()
;         EndIf
;       EndIf
;     EndIf
;   Next
...
;   ForEach UM_Database()
;     test(UM_Database()\UM_WHD_Entry+".html")=UM_Database()\UM_WHD_Entry+".html"
;   Next
;   
;   While Not Eof(i)
;     path=ReadString(i)
;     If Not FindMapElement(test(),path)
;       If UM_Database()\UM_Type="Demo" Or UM_Database()\UM_Beta_Type="Demo"
;         Debug path
;       EndIf
;     EndIf
    ...
    Case #PB_Event_Menu
      ...
      Select EventMenu()
        ...
        Case #MenuItem_5   ;{- Quit
          If MessageRequester("Exit UltraMiggy", "Do you want to quit?",#PB_MessageRequester_YesNo|#PB_MessageRequester_Warning,WindowID(#MAIN_WINDOW))=#PB_MessageRequester_Yes
            Break
          EndIf ;}
        ...
        Case #MenuItem_28  ;{- Extract text files
          Make_AGS_Guide(Home_Path+"Games.guide",#False)
          Make_AGS_Demos_Guide(Home_Path+"Demos.guide",#False)
...
; CursorPosition = 9444
; FirstLine = 1855
; Folding = AAAAAAAAAAAAAIEAAEACBAAAAAAAAAAAA+
; Optimizer
; EnableThread
; EnableXP
; DPIAware
...
; Compiler = PureBasic 6.10 beta 6 (Windows - x64)
; Debugger = Standalone