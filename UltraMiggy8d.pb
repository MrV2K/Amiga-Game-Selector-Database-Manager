...
Macro Generate_Images()
  ...
  If response="4" Or response="6" ; PC
    source_file$=#DOUBLEQUOTE$+source_folder$+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Icon,#PB_FileSystem_NoExtension)+"_Cover.png"+#DOUBLEQUOTE$
    If FileSize(RemoveString(source_file$,#DOUBLEQUOTE$))>0
      new_file$=#DOUBLEQUOTE$+dest_folder$+"PC_Images\Covers\"+UM_Database()\UM_Type+"\"+GetFilePart(UM_Database()\UM_Slave,#PB_FileSystem_NoExtension)+".png"+#DOUBLEQUOTE$
      path="-quiet -overwrite -resize 640 824 -out png -o "+new_file$+" "+source_file$
      RunProgram(nconvert_path,path,"",#PB_Program_Wait)
    EndIf
    source_file$=#DOUBLEQUOTE$+source_folder$+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Icon,#PB_FileSystem_NoExtension)+"_Screen.png"+#DOUBLEQUOTE$
    If FileSize(RemoveString(source_file$,#DOUBLEQUOTE$))>0
  ...
  If response="5" Or response="6" ; Launchbox
    source_file$=#DOUBLEQUOTE$+source_folder$+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Icon,#PB_FileSystem_NoExtension)+"_Cover.png"+#DOUBLEQUOTE$
    If FileSize(RemoveString(source_file$,#DOUBLEQUOTE$))>0
      new_file$=#DOUBLEQUOTE$+dest_folder$+"LaunchBox\Covers\"+UM_Database()\UM_Type+"\"+GetFilePart(UM_Database()\UM_LHAFile,#PB_FileSystem_NoExtension)+".png"+#DOUBLEQUOTE$
      path="-quiet -overwrite -clevel 9 -resize 640 824 -rtype lanczos -out png -o "+new_file$+" "+source_file$
      RunProgram(nconvert_path,path,"",#PB_Program_Wait)
    EndIf
    source_file$=#DOUBLEQUOTE$+source_folder$+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Icon,#PB_FileSystem_NoExtension)+"_Screen.png"+#DOUBLEQUOTE$
    If FileSize(RemoveString(source_file$,#DOUBLEQUOTE$))>0
    ...
    source_file$=#DOUBLEQUOTE$+source_folder$+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Icon,#PB_FileSystem_NoExtension)+"_Cover.png"+#DOUBLEQUOTE$
    If FileSize(RemoveString(source_file$,#DOUBLEQUOTE$))>0
      new_file$=#DOUBLEQUOTE$+dest_folder$+"LaunchBox_UM\Covers\"+UM_Database()\UM_Type+"\"+UM_Database()\UM_Title+Tags()+".png"+#DOUBLEQUOTE$
      path="-quiet -overwrite -clevel 9 -resize 640 824 -rtype lanczos -out png -o "+new_file$+" "+source_file$
      RunProgram(nconvert_path,path,"",#PB_Program_Wait)
    EndIf
    source_file$=#DOUBLEQUOTE$+source_folder$+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Icon,#PB_FileSystem_NoExtension)+"_Screen.png"+#DOUBLEQUOTE$
...
Procedure Scan_HTTP()
  ...
  List_Files_Recursive(Dats_Path,Dat_List(),"*.zip")
...
Procedure Update_FTP()
  ...
  If ListSize(Down_List())>0
    ...
    ForEach Down_List()   
      ...
      Download_Path=WHD_Folder+Down_List()
      Server_Path=HTTP_Server+"/"+http_path+"/"+StringField(Down_List(),2,"\")+"/"+StringField(Down_List(),3,"\")
      
      If ReceiveHTTPFile(Server_Path,Download_Path)  
        PrintN("Downloaded (" + Str(ListIndex(Down_List())+1) + " of " + Str(ListSize(Down_List())) + ") - " + Down_List() + " (" + Str(FileSize(Download_Path)) + " bytes)")  
      Else
        PrintNCol("*** Error ***",4,0)
      EndIf 
...
Procedure Generate_IGame_Data(dest_folder$)
  ...
  If dest_folder$<>""
    ...
    ForEach UM_Database()
      If FileSize(dest_folder$+UM_Database()\UM_Type)<>-2 : CreateDirectory(dest_folder$+UM_Database()\UM_Type) : EndIf
      If FileSize(dest_folder$+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder)<>-2 : CreateDirectory(dest_folder$+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder) : EndIf
      If FileSize(dest_folder$+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder)<>-2 : CreateDirectory(dest_folder$+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder) : EndIf
      IGame_Data_Entry()
...
Procedure Edit_GL(number) 
  ...
  Protected Next_Button, Prev_Button, Publisher_Text, Alt_Name_String, Alt_Name_Short_String, WHD_Button, Restricted_Check, Data_Disk, Adult_Check
  
  old_num=GetGadgetState(#MAIN_LIST)
  
  DisableWindow(#MAIN_WINDOW,#True)
  
  If OpenWindow(#EDIT_WINDOW, 0, 0, 424, 590, "Edit Database ("+Str(ListIndex(List_Numbers()))+" of "+Str(ListSize(List_Numbers()))+")", #PB_Window_SystemMenu | #PB_Window_WindowCentered, WindowID(#MAIN_WINDOW))
    ...
    TextGadget(#PB_Any, 158, 408, 50, 20, "Config")  
    Config_Combo = ComboBoxGadget(#PB_Any, 212, 406, 124, 24)
...
; CursorPosition = 3380
; FirstLine = 1055
; Folding = AAAAAAAAAAQAAAAAAAAAASAAAAAAAAA9
; Optimizer
; EnableThread
; EnableXP
; DPIAware
