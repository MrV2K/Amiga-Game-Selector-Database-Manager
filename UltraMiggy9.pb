    ...
    While NextDirectoryEntry(Folder_LIST)
      Select DirectoryEntryType(Folder_LIST)
        Case #PB_DirectoryEntry_File
          ...
          Debug count
          Debug Files()\R_File_Name
          Debug Files()\R_File_File
          Files()\R_File_Size = DirectoryEntrySize(Folder_LIST)
        Case #PB_DirectoryEntry_Directory
          Select DirectoryEntryName(Folder_LIST)
            Case ".", ".."
...
Procedure Draw_List()
  ...
  Protected game.i, demo.i, beta.i, icon.i, extras.s, magazine.i, type.s
...
Procedure Update_HDF()
  ...
  If CreateDirectory(WHD_TempDir)
    ...
    DeleteFile(home_path+"whd-startup")
    ...
    If startup_file
      ...
      WriteString(startup_file," RequestChoice Error "+#DOUBLEQUOTE$+"WHD-HDD Not Available. "+#DOUBLEQUOTE$+"OK"+#LF$)
    ...
    If FileSize(WHD_TempDir+"complete.txt")=0 Or FileSize(WHD_TempDir+"complete.txt")=-1
      DeleteFile(WHD_TempDir+"dirs.txt")
      DeleteFile(WHD_TempDir+"slaves.txt")
      DeleteFile(WHD_TempDir+"whd-startup")
      DeleteDirectory(WHD_TempDir,"*.*",#PB_FileSystem_Recursive|#PB_FileSystem_Force)
    ...
    FileSize(home_path+"slaves.txt")
    
    ; Copy the new files into the home folder
    
    If FileSize("dirs.txt")>0 : CopyFile("dirs.txt",home_path+"dirs.txt") : Else : Goto Cleanup : EndIf
    If FileSize("slaves.txt")>0 : CopyFile("slaves.txt",home_path+"slaves.txt") : Else : Goto Cleanup : EndIf
    SetCurrentDirectory(home_path)
...
Procedure Compare_AGS_Folders()
  ...
  Protected NewList Compare_Files.File_Data()
  Protected NewList New_Files.File_Data()
  Protected NewMap Compare_Map.i()
  
  Protected compare_path.s, new_path.s
  
  compare_path=PathRequester("Compare Path", Home_Path)
  List_Files_Recursive_Size_Only(compare_path,Compare_Files(),"*.*")
  
  new_path=PathRequester("New Path", Home_Path)
  List_Files_Recursive_Size_Only(new_path,New_Files(),"*.*")
  
  ForEach Compare_Files()
    Compare_Map(Compare_Files()\R_File_Name)=Compare_Files()\R_File_Size
  Next
    
  ForEach New_Files()
    If Not FindMapElement(Compare_Map(),New_Files()\R_File_Name)
      Debug New_Files()\R_File_Name
    EndIf
  Next
...
Macro Make_AGS_Image(image_path)
  ...
  If make_images
    ...
    If Not ECS ; Don't create titles image for ECS version
...
Macro Add_Check()
  ...
  If Not ECS ; Don't add ECS Title to check
   AddElement(Check_List())
...
Procedure AGS_Window()
  ...
  EndIf
  If Not UM_Database()\UM_7Mhz And Not UM_Database()\UM_14Mhz And Not UM_Database()\UM_25Mhz
      ...
      Case save_button     
        ...
        If EventWindow()=AGSWindow And type=#PB_EventType_LeftClick
          If GetGadgetState(CPUSpeed_Combo)=0
            UM_Database()\UM_7Mhz=#False : UM_Database()\UM_14Mhz=#False : UM_Database()\UM_25Mhz=#False
          EndIf
          If GetGadgetState(CPUSpeed_Combo)=1
            UM_Database()\UM_7Mhz=#True : UM_Database()\UM_14Mhz=#False : UM_Database()\UM_25Mhz=#False
          EndIf
          If GetGadgetState(CPUSpeed_Combo)=2
            UM_Database()\UM_7Mhz=#False : UM_Database()\UM_14Mhz=#True : UM_Database()\UM_25Mhz=#False
          EndIf
          If GetGadgetState(CPUSpeed_Combo)=3
            UM_Database()\UM_7Mhz=#False : UM_Database()\UM_14Mhz=#False : UM_Database()\UM_25Mhz=#True
          EndIf
...
Procedure Edit_Database(number) 
  ...
  If OpenWindow(#EDIT_WINDOW, 0, 0, 450, 590, "Edit Database ("+Str(ListIndex(List_Numbers()))+" of "+Str(ListSize(List_Numbers()))+")", #PB_Window_SystemMenu | #PB_Window_WindowCentered, WindowID(#MAIN_WINDOW))
    ...
    AGS_Button=ButtonIconGadget(#PB_Any,88,367,110,22,"AGS Settings",ImageID(#AGS_ICON),16,16)
...
Procedure Sort_Main_List(Column.i)
...
EndDataSection
; IDE Options = PureBasic 6.12 beta 2 LTS (Windows - x64)
; CursorPosition = 11877
; FirstLine = 1721
; Folding = AAAAAAAAAAAAAAAQAAAAAAQAAAAAAAAAAA+
; Optimizer
; EnableThread
; EnableXP
; DPIAware
...
; Compiler = PureBasic 6.11 LTS Beta 3 (Windows - x64)
; Debugger = Standalone