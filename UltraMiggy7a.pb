...
; 21. Genres now loaded from main database in filter window.
...
; 4. Update file structure to match WHDLoad Downloader.
; 5. Move over WHDLoad downloader refined code to update procedures
;
;============================================
...
Procedure Draw_Game_Info(*nul)
  ...
  If GetGadgetState(#MAIN_LIST)<>-1
    ...
    output$+LSet("Install",11," ")+": "+UM_Database()\UM_InstallSize+#CRLF$
    If UM_Database()\UM_ECSOCS=#True
...
Procedure Save_IG_DB()
  ...
  If IG_File
    ForEach UM_Database()
      WriteStringN(IG_File,UM_Database()\UM_Slave+";"+UM_Database()\UM_Folder+";"+UM_Database()\UM_Genre+";"+UM_Database()\UM_Title+Tags()+";"+UM_Database()\UM_Short+Tags_Short()+";"+UM_Database()\UM_Default_Icon+";"+UM_Database()\UM_LHAFile)
    Next
  EndIf
...
Procedure Update_Database()
  ...
  If ListSize(File_List())>0 
    ...
    ForEach UM_Database() 
      Folder_Check(LCase(UM_Database()\UM_Type+"_"+UM_Database()\UM_Folder+"_"+UM_Database()\UM_Default_Icon))=ListIndex(UM_Database()) ; Map to check if folder exists.
      Check_Archives(LCase(UM_Database()\UM_Type+"_"+UM_Database()\UM_LHAFile))=0                                                       ; Map to check if WHD archive exists
    Next
    ...
    ForEach File_List()
      ...
      path2=LCase(StringField(File_List(),count-1,"\")) ; Get file type from path
    ...
    ForEach Check_List() 
      ...
      PrintN("Scanning Icons: "+Check_List() )
      ...
      If UM_Program   
        ...
        For i=1 To count
          ...
          If FindString(ReadO$,".info",0,#PB_String_NoCase) Or FindString(ReadO$,".slave",0,#PB_String_NoCase)               
            If GetExtensionPart(Check_List())="lha"
              zeros=53
            EndIf
            If GetExtensionPart(Check_List())="lzx"
            ...
            If CountString(path,"/")=1
              ...
              If FindString(path,".slave",0,#PB_String_NoCase)  
                date$=Process_Date(GetExtensionPart(Check_List()),ReadO$)
              EndIf
            Else
              Continue
            EndIf
      ...
      ForEach Copy_List()
        ...
        path=LCase(StringField(Check_List(),count-1,"\")+"_"+Copy_List()\Copy_Folder+"_"+Copy_List()\Copy_File)
        If Not FindMapElement(Folder_Check(),path)
          AddElement(UM_Database())
          UM_Database()\UM_Subfolder=StringField(Check_List(),count,"\")
          UM_Database()\UM_LHAFile=StringField(Check_List(),count+1,"\")
          UM_Database()\UM_Type=StringField(Check_List(),count-1,"\")
          If UM_Database()\UM_Type="Beta" : UM_Database()\UM_Beta=#True : EndIf
          UM_Database()\UM_Default_Icon=Copy_List()\Copy_File
          ...
          UM_Database()\UM_Language="English"
          ...
          UM_Database()\UM_Title=Split_On_Capital(GetFilePart(Check_List(),#PB_FileSystem_NoExtension))
        ...
        ForEach UM_Database() 
          ...
          If response="1" Or response="5"
            ...
            If FileSize(RemoveString(source_file$,#DOUBLEQUOTE$))>0
              ...
              path="-quiet -overwrite -out iff -o "+new_file$+" -c 1 -colours 256 "+source_file$
              RunProgram(nconvert_path,path,"",#PB_Program_Wait)
            EndIf
          ...
          If response="4"  Or response="5"
            ...
            If FileSize(RemoveString(source_file$,#DOUBLEQUOTE$))>0
              ...
              path="-quiet -overwrite -clevel 6 -out png -o "+new_file$+" "+source_file$
              RunProgram(nconvert_path,path,"",#PB_Program_Wait)
            EndIf
            source_file$=#DOUBLEQUOTE$+source_folder$+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension)+"_Title.png"+#DOUBLEQUOTE$
            If FileSize(RemoveString(source_file$,#DOUBLEQUOTE$))>0
          ...
          If response="6"
            ...
            If FileSize(RemoveString(source_file$,#DOUBLEQUOTE$))>0
              ...
              path="-quiet -overwrite -out iff -o "+new_file$+" -c 1 "+source_file$
              RunProgram(nconvert_path,path,"",#PB_Program_Wait)
            EndIf
            source_file$=#DOUBLEQUOTE$+source_folder$+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension)+"_Screen.png"+#DOUBLEQUOTE$
            If FileSize(RemoveString(source_file$,#DOUBLEQUOTE$))>0
...
Procedure Image_Popup(type.i)
  ...
  If GetGadgetState(#MAIN_LIST)>-1
    ...
    Select GetGadgetState(#MAIN_PANEL)
      ...
      Case 0 : path=Game_Data_Path+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension)+extension     
...
Procedure Edit_GL(number) 
  ...
  If OpenWindow(#EDIT_WINDOW, 0, 0, 424, 530, "Edit Database", #PB_Window_SystemMenu | #PB_Window_WindowCentered,WindowID(#MAIN_WINDOW))
    ...
    Icon_String = StringGadget(#PB_Any, 80, 136, 336, 24, UM_Database()\UM_Default_Icon)
    If UM_Database()\UM_Type="Demo"
    ...
    Cancel_Button = ButtonGadget(#PB_Any, 270, 498, 72, 24, "Cancel")
    Close_Button = ButtonGadget(#PB_Any, 344, 498, 72, 24, "Close")
    ...
    If change3
      old_pos=GetGadgetText(#MAIN_LIST)
      Draw_List() 
      Center_List(old_pos)
    Else  
      SetActiveGadget(#MAIN_LIST)
...
Procedure Filter_Window()
  ...
  Protected g_file.i, old_pos.i, text.i, x_pos.i
  ...
  If OpenWindow(#FILTER_WINDOW, x_pos, WindowY(#MAIN_WINDOW)+30, 325, 650, "Filter", #PB_Window_SystemMenu)
    ...
    AddGadgetItem(Genre_Gadget,-1,"All")
    ForEach UM_Database()
      Genre_Map(UM_Database()\UM_Genre)=UM_Database()\UM_Genre
    Next
    ForEach Genre_Map()
      AddElement(Genre_List())
...
;- ############### Gadget Events    
    ...
    Case #PB_Event_CloseWindow
      Select EventWindow()
        ...
        Case #FILTER_WINDOW  : CloseWindow(#FILTER_WINDOW)
...
; CursorPosition = 4004
; FirstLine = 995
; Folding = AAAAgBAAAAAADCAAABAAAAAAA+
; Optimizer
; EnableThread
; EnableXP
; EnableUser
