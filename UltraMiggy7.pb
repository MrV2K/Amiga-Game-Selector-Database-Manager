...
;============================================
;
; 1. Add full icon support by scanning .info files for slaves and adding to DB. Use either default icon on all icons.
;
;============================================
...
Global W_Title.s="UltraMiggy v0.7"
...
Procedure Draw_Game_Info(*nul)
  ...
  If GetGadgetState(#MAIN_LIST)<>-1
    ...
    output$+LSet("Def. Icon",11," ")+": "+UM_Database()\UM_Default_Icon+#CRLF$
    If UM_Database()\UM_ECSOCS=#True
...
Procedure Create_Bezel(type.i)
  ...
  Protected output_path.s, output_type.s, bezel_name.s, screen_png.i, graphics_png.i, output_image.i, left_image.i ,y.i, right_image.i
  
  Select type
    Case 1 : output_type=Game_Data_Path+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension)+"_Screen.png"
    Case 2 : output_type=Game_Data_Path+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension)+"_Title.png"
    Case 3 : output_type=Game_Data_Path+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension)+"_Cover.png"
  EndSelect
  ...
  graphics_png=LoadImage(#PB_Any,output_type)
...
;   FreeImage(left_image)
;   FreeImage(right_image)
...
Procedure Run_Game(gamenumber.i)
  ...
  If CreateDirectory(WHD_TempDir)
    ...
    If startup_file
      ...
      WriteString(startup_file,"echo "+#DOUBLEQUOTE$+""+#DOUBLEQUOTE$+#LF$)
      WriteString(startup_file,"echo "+#DOUBLEQUOTE$+UM_Database()\UM_Title+Tags()+#DOUBLEQUOTE$+#LF$)
...
Procedure Run_Icon(gamenumber.i, icon_name.s)
  ...
  If CreateDirectory(WHD_TempDir)
    ...
    Select UM_Database()\UM_Config
      Case 0 : config=IG_Path+"A1200_WHD.uae"
      Case 1 : config=IG_Path+"A1200_WHD_030.uae"
      Case 2 : config=IG_Path+"A1200_WHD_040.uae"
      Case 3 : config=IG_Path+"A1200_WHD_CD32.uae"
    EndSelect
...
Procedure Audit_Archives()
  ...
  If OpenFTP(#FTP,ftp_server,ftp_user,ftp_pass,ftp_passive,ftp_port)
    ...
    PrintS()  
    PrintNCol("Scanning Database",9,0)
    
    ForEach UM_Database()
      If FindMapElement(Comp_Map(),UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_LHAFile)=-1
        PrintNCol("Error in: "+MapKey(Comp_Map()),9,0)
      EndIf
    Next
...
Procedure Compress_Folder(c_path.s, c_file.s)
...
Procedure.s Process_Date(type.s,string.s)
...
Procedure Update_FTP()
 ...
 ;{ 1. Download dat files #####################################################################################################################################################################  
  ...
  Else
    ...
    Delay(1000)
;    PrintS()
;     If AskYN("Select Manually?")
;       name$=OpenFileRequester("Select DAT Files...","","*.zip",-1,#PB_Requester_MultiSelection)
;       While name$
;         Debug name$
;         CopyFile(name$,tempfolder+GetFilePart(name$))
;         AddElement(Dat_List())
;         Dat_List()=tempfolder+GetFilePart(name$)
...
Procedure Update_Database()
  ...
  Protected ReadO$, Output$, SubFolder$, length.i, UM_Program.i
  Protected oldpath.s
  ...
  If ListSize(File_List())>0 
    ...
    ForEach Check_List() 
      ...
      PrintN("Processing "+GetFilePart(Check_List())+" ("+ListIndex(Check_List())+" of "+ListSize(Check_List())+")") ; Display which archive is being processed
      ...
      If GetExtensionPart(Check_List())="lha"
        length=54
        i=1
        Debug LHA_Path+" l "+#DOUBLEQUOTE$+Check_List()+#DOUBLEQUOTE$
      ...
      If UM_Program ; Whilst the archiver is running, capture the output and process data
        While ProgramRunning(UM_Program)         
          If AvailableProgramOutput(UM_Program)  
            ReadO$+ReadProgramString(UM_Program)+#CRLF$        
          EndIf 
        Wend
        CloseProgram(UM_Program)
      Else
      ...
      count=CountString(ReadO$,#CRLF$)
      
      For i=1 To count ; Create Icon List
        path=StringField(ReadO$,i,#CRLF$)
        path=Mid(path,length,Len(path)) ; Cut Out Icon Path
        If FindString(path,".info",0,#PB_String_NoCase)
          If CountString(path,"\")=1 
            AddElement(Icon_List())
            Icon_List()=GetFilePart(path)
          EndIf
        EndIf
      Next
      ...
      For i=1 To count ; Create Icon List
        ...
        If FindString(path,".slave",0,#PB_String_NoCase)
          If CountString(path,"\")=1 ; Only process slave files in the main directory            
            AddElement(Add_List())   ; If the archiver is running start adding data to the Add List
            Add_List()\add_date=Process_Date(GetExtensionPart(Check_List()),path) ; Get Slave Date For DB based on the WHD archive extension                       
            path=Mid(path,length,Len(path)) ; Cut Out Slave Path
            Add_List()\add_sub=SubFolder$ ; Add subfolder to Add List
            Add_List()\add_type=StringField(Check_List(),4,"\") ; Add archive type to Add List
            Add_List()\add_arc=GetFilePart(Check_List())        ; Add WHD archive name to Add List
            Add_List()\add_full_arc=Check_List()                ; Add full WHD archive path to Add List
            Add_List()\add_file=path                            ; Add the slave name to the Add List entry    
            CopyList(Icon_List(),Add_List()\Icons2())      
          EndIf  
        EndIf
      Next
    Next 
    ...
    oldpath=Game_Data_Path+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension)
    ...
    ForEach Add_List()
      ...
      If FindMapElement(Folder_Check(),LCase(Add_List()\add_type+"_"+RemoveString(GetPathPart(Add_List()\add_file),"/"))) 
        ...
        SelectElement(UM_Database(),Folder_Check()) ;Select the database entry to change
        UM_Database()\UM_LHAFile=Add_List()\add_arc ; Update WHD archive name
        UM_Database()\UM_Slave_Date=Add_List()\add_date ; Update the slave date to the new one.

        oldpath=Game_Data_Path+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension) ; Store the existing icon name for the file rename later

        ClearList(UM_Database()\UM_Icons()) ; Clear the existing list of icons
        CopyList(Add_List()\Icons2(), UM_Database()\UM_Icons()) ; Copy the new list of icons into the existing database entry
        
        Protected NewMap icon_map.s()

        ForEach UM_Database()\UM_Icons() ; Copy the new scanned icons into a map for comparison against the existing default icon
          icon_map(UM_Database()\UM_Icons())=UM_Database()\UM_Icons()
        Next

        If Not FindMapElement(icon_map(),UM_Database()\UM_Default_Icon) ; If the default icon isn't in the map, ask for a new one.

          PrintS()
          PrintN("Choose Default Icon for "+UM_Database()\UM_Slave) ; Generate the test for selecting a new default icon
          PrintS()
          count=1
          ForEach UM_Database()\UM_Icons()
          ...
          SelectElement(UM_Database()\UM_Icons(),i-1) ; Select the icon list entry based on the input.

          UM_Database()\UM_Default_Icon=UM_Database()\UM_Icons() ; Copy the entry into the database default icon entry
          
          ; Rename the old images to the new default icon name
          
          path=Game_Data_Path+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension)
          
          RenameFile(oldpath+"_Screen.png",path+"_Screen.png")
          RenameFile(oldpath+"_Title.png",path+"_Title.png")
          RenameFile(oldpath+"_Cover.png",path+"_Cover.png")
    ...
    ForEach Add_List()
      ...
      If Not FindMapElement(Folder_Check(),LCase(Add_List()\add_type+"_"+RemoveString(GetPathPart(Add_List()\add_file),"/"))) 
        ...
        UM_Database()\UM_Title=RemoveString(UM_Database()\UM_Title,"\")
        UM_Database()\UM_Folder=RemoveString(GetPathPart(Add_List()\add_file),"\")
        ...
        If FindString(Add_List()\add_file,"CDTV") : UM_Database()\UM_CDTV=#True : EndIf
        ...
        ForEach UM_Database()\UM_Icons() ; Set default icon based on slave name
          If GetFilePart(UM_Database()\UM_Icons(),#PB_FileSystem_NoExtension)=GetFilePart(UM_Database()\UM_Slave,#PB_FileSystem_NoExtension)
            UM_Database()\UM_Default_Icon=UM_Database()\UM_Icons()
            Break
          EndIf
        Next
        
        If UM_Database()\UM_Default_Icon=""
          PrintS()
          PrintN("Choose Default Icon for "+UM_Database()\UM_Slave)
          PrintS()
          count=1
          ForEach UM_Database()\UM_Icons()
            PrintN(Str(count)+": "+UM_Database()\UM_Icons())
...
Procedure Update_HDF()
  ...
  If cd_file
    ...
    ForEach Miggy_List()
      If FindString(Miggy_List(),"newyork",#PB_Ignore,#PB_String_NoCase): Debug path : EndIf
  ...
  ForEach UM_Database()
    ...
    If FileSize(WHD_Folder+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_LHAFile)=-1
      ...
      If path2="y" Or path2="Y"
        ...
        Compress_Folder(olddir,Backup_Path+UM_Database()\UM_Folder+".zip")
...
Procedure Make_PD_Disk(out_path.s="")
  ...
  If UM_Database()\UM_Type="Demo"
    ...
    LoadImage(#BACK_IMAGE,Game_Data_Path+"Demo\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension)+"_Title.png")      
    ...
    SaveImage(#TEMP_IMAGE,GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension)+"_Cover.png",#PB_ImagePlugin_PNG)
...
Procedure Audit_Images()
  ...
  ForEach UM_Database()
    path=Game_Data_Path+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension)+"_Screen.png"
    DB_Map(LCase(path))=path
    path=Game_Data_Path+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension)+"_Title.png"
    DB_Map(LCase(path))=path
    path=Game_Data_Path+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension)+"_Cover.png"
    DB_Map(LCase(path))=path
  Next
...
Procedure Backup_Images()
  ...
  result=CopyFile(Game_Data_Path+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension)+"_Cover.png",path2+"\"+GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension)+"_Cover.png")
  result=CopyFile(Game_Data_Path+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension)+"_Screen.png",path2+"\"+GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension)+"_Screen.png")
  result=CopyFile(Game_Data_Path+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension)+"_Title.png",path2+"\"+GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension)+"_Title.png")
...
Procedure.s Check_Icon_File(file.s)
...
Procedure Scan_Single_Icon()
  
  Protected NewList Copy_List.s()
  Protected NewList Icon_List.Icon_Data()
  
  Protected UM_Program.i, zeros.i
  Protected ReadO$, oldpath.s, command.s, Archiver.s, response.s

  CreateDirectory(WHD_TempDir)
  
  OpenConsole("Scanning Icons...")
  
  PrintN("Scanning Icons: "+UM_Database()\UM_LHAFile)
  ...
  If UM_Program     
    While ProgramRunning(UM_Program)
      If AvailableProgramOutput(UM_Program) 
        ReadO$=ReadProgramString(UM_Program,#PB_Ascii)             
        ...
        If FindString(ReadO$,".info",0,#PB_String_NoCase)
          ...
          If GetExtensionPart(UM_Database()\UM_LHAFile)="lzx"
            zeros=49
          EndIf
          path=Right(ReadO$,Len(ReadO$)-zeros)
          path=RemoveString(path,#DOUBLEQUOTE$)
          If CountString(path,"/")=1
            AddElement(Icon_List())
            Icon_List()\Icon_File=path
          Else
            Continue
          EndIf
          path=StringField(path,2,"/")
          Icon_List()\Icon_Name=path
        EndIf 
      EndIf               
    Wend 
  EndIf 
  ...
  ForEach Icon_List()
    ...
    If response<>""
      AddElement(Copy_List())
      Copy_List()=GetFilePart(path)
    EndIf
  Next
  
  ClearList(UM_Database()\UM_Icons())       
  CopyList(Copy_List(),UM_Database()\UM_Icons())
...
Procedure Scan_Icons()
  ...
  ForEach UM_Database()
    ...
    EndIf 
      ...
      ForEach Icon_List()
        PrintN("Extracting: "+Icon_List()\Icon_Name+" - "+Icon_List()\Icon_File)
        If GetExtensionPart(UM_Database()\UM_LHAFile)="lha"
          command="x "+path+" -o"+WHD_TempDir+" "+#DOUBLEQUOTE$+Icon_List()\Icon_File+#DOUBLEQUOTE$
          UM_Program=RunProgram(LHA_Path,command,GetCurrentDirectory(),#PB_Program_Wait)
        EndIf
      ...
      If UM_Database()\UM_Default_Icon="" : SelectElement(UM_Database()\UM_Icons(),0) : UM_Database()\UM_Default_Icon=UM_Database()\UM_Icons() : EndIf
...
Procedure Create_AGS_Files()
  ...
  ForEach UM_Database()
      ...
      script+"kgiconload "+UM_Database()\UM_Default_Icon+#LF$
      script+"AGS:"+#LF$
      script+"cls"+#LF$
      ...
      source_file$=#DOUBLEQUOTE$+source_folder$+GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension)+"_Cover.png"+#DOUBLEQUOTE$
      ...
      source_file$=#DOUBLEQUOTE$+source_folder$+GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension)+"_Title.png"+#DOUBLEQUOTE$
      ...
      source_file$=#DOUBLEQUOTE$+source_folder$+GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension)+"_Screen.png"+#DOUBLEQUOTE$
...
Procedure Create_IFF_Folders()
  ...
  source_folder$=PathRequester("Select Source Folder",Home_Path)  
  If source_folder$<>""
      ...
      If LCase(response)<>"c"
        ForEach UM_Database() 
          ...
          If response="1" Or response="5"
            source_file$=#DOUBLEQUOTE$+source_folder$+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension)+"_Cover.png"+#DOUBLEQUOTE$
            If FileSize(RemoveString(source_file$,#DOUBLEQUOTE$))>0
              ...
              RunProgram(nconvert_path,path,"",#PB_Program_Wait)
            EndIf
            source_file$=#DOUBLEQUOTE$+source_folder$+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension)+"_Screen.png"+#DOUBLEQUOTE$
            If FileSize(RemoveString(source_file$,#DOUBLEQUOTE$))>0
              ...
              RunProgram(nconvert_path,path,"",#PB_Program_Wait)
            EndIf
            source_file$=#DOUBLEQUOTE$+source_folder$+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension)+"_Title.png"+#DOUBLEQUOTE$
            If FileSize(RemoveString(source_file$,#DOUBLEQUOTE$))>0
          ...
          If response="2" Or response="5"
            ...
            If FileSize(RemoveString(source_file$,#DOUBLEQUOTE$))>0
              ...
              path="-quiet -out iff -o "+new_file$+" -c 1 -ratio -resize 320 256 -rtype lanczos -canvas 320 256 center -bgcolor 0 0 0 -resize 320 128 -colors 16 -floyd "+source_file$
              RunProgram(nconvert_path,path,"",#PB_Program_Wait)
              ...
              path="-quiet -out iff -o "+new_file$+" -c 1 -ratio -resize 320 256 -rtype lanczos -canvas 320 256 center -bgcolor 0 0 0 -resize 320 128 -colors 216 -floyd "+source_file$
              RunProgram(nconvert_path,path,"",#PB_Program_Wait)
              ...
              path="-quiet -out iff -o "+new_file$+" -c 1 -ratio -resize 640 400 -rtype lanczos -canvas 640 400 center -bgcolor 0 0 0 -resize 640 400 -colors 16 -floyd "+source_file$
              RunProgram(nconvert_path,path,"",#PB_Program_Wait)
              ...
              path="-quiet -out iff -o "+new_file$+" -c 1 -ratio -resize 640 400 -rtype lanczos -canvas 640 400 center -bgcolor 0 0 0 -resize 640 400 -colors 216 -floyd "+source_file$
              RunProgram(nconvert_path,path,"",#PB_Program_Wait)
            EndIf
            source_file$=#DOUBLEQUOTE$+source_folder$+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension)+"_Screen.png"+#DOUBLEQUOTE$
            If FileSize(RemoveString(source_file$,#DOUBLEQUOTE$))>0
...
Procedure Check_Missing_Images(type.i)
  ...
  ForEach UM_Database()  
    ...
    If type=2
      path2=Game_Data_Path+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension)+"_Cover.png"
      If FileSize(path2)>0
...
Procedure Image_Popup(type.i)
  ...
  If GetGadgetState(#MAIN_LIST)>-1
    ...
    If GetGadgetState(#MAIN_PANEL)=1
      If type=1
        path=Game_Data_Path+"CD32\Titles\"+CD32_Database()\CD_Title+".png"
        ww=640 : wh=512
      EndIf
      
      If type=2
        path=Game_Data_Path+"CD32\Screenshots\"+CD32_Database()\CD_Title+".png"
        ww=640 : wh=512
      EndIf
      
      If type=3
        path=Game_Data_Path+"CD32\Covers\"+CD32_Database()\CD_Title+".png"
        ww=640 : wh=824
      EndIf
...
Procedure Text_Viewer(file.s)
  ...
  If IsWindow(#TEXTVIEW_WINDOW)
    SetGadgetText(#TEXTVIEW_EDITOR,"")
    SetWindowTitle(#TEXTVIEW_WINDOW,"Text Viewer ("+GetFilePart(file)+")")
  Else
    OpenWindow(#TEXTVIEW_WINDOW,0,0,600,650,"Text Viewer ("+GetFilePart(file)+")",#PB_Window_SystemMenu|#PB_Window_WindowCentered,WindowID(#MAIN_WINDOW))
    EditorGadget(#TEXTVIEW_EDITOR,0,0,600,650,#PB_Editor_ReadOnly|#PB_Editor_WordWrap)
    DestroyCaret_()
    SetGadgetFont(#TEXTVIEW_EDITOR,FontID(#INFO_FONT))
  EndIf
  ...
  If IsGadget(#TEXTVIEW_EDITOR)
    SetGadgetText(#TEXTVIEW_EDITOR,output2$)
  EndIf
  
  SetActiveWindow(#TEXTVIEW_WINDOW)
  
EndProcedure  

Procedure PDF_Viewer(file.s)
  
  Protected output$, output2$
    
  If IsWindow(#PDF_WINDOW)
    Pause_Window(#PDF_WINDOW)
    If IsGadget(#PDF_WEBGADGET) : FreeGadget(#PDF_WEBGADGET) : EndIf
    WebGadget(#PDF_WEBGADGET,0,0,600,650,"file://"+file)
    Resume_Window(#PDF_WINDOW)
  Else
    OpenWindow(#PDF_WINDOW,0,0,600,650,"PDF Viewer ("+GetFilePart(file)+")",#PB_Window_SystemMenu|#PB_Window_SizeGadget|#PB_Window_MinimizeGadget|#PB_Window_MaximizeGadget|#PB_Window_WindowCentered,WindowID(#MAIN_WINDOW))
    Pause_Window(#PDF_WINDOW)
    WebGadget(#PDF_WEBGADGET,0,0,600,650,"file://"+file)
    Resume_Window(#PDF_WINDOW)
  EndIf
...
Procedure Save_IG_DB()
  ...
  If IG_File
    ForEach UM_Database()
      WriteStringN(IG_File,UM_Database()\UM_Slave+";"+UM_Database()\UM_Folder+";"+UM_Database()\UM_Genre+";"+UM_Database()\UM_Title+Tags()+";"+UM_Database()\UM_Short+Tags_Short()+";"+UM_Database()\UM_Default_Icon)
    Next
  EndIf
...
Procedure Choose_Icon()
  ...
  If Val(path)>0 And Val(path)<ListSize(Icon_List())+1 And LCase(path)<>"c"
    ...
    If i<ListSize(Icon_List())+1
      ...
      AddElement(UM_Database()\UM_Icons())
      UM_Database()\UM_Icons()=UM_Database()\UM_Default_Icon
      path=Game_Data_Path+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension)
      RenameFile(oldpath+"_Screen.png",path+"_Screen.png")
      RenameFile(oldpath+"_Title.png",path+"_Title.png")
      RenameFile(oldpath+"_Cover.png",path+"_Cover.png")
...
;             Case ISO_String
;               If EventType()=#PB_EventType_Change
;                 CD32_Database()\CD_File=GetGadgetText(ISO_String)
;                 change=#True
;               EndIf
            Case ISO_Button
...
Procedure Edit_GL(number) 
  ...
  Protected change.b=#False, change3.b=#False, oldindex.s, star.s, g_file.i, text$, old_num, count, old_path.s
  ...
  Protected CDTV_Check, MT32_Check, Memory_String, Genre_Combo, CDROM_Check, Arcadia_Check, Demo_Check, Intro_Check
  Protected Cover_Check, NTSC_Check, Disk_String, Coder_String, Type_Combo, File_Button, Config_Combo
  Protected Year_String, Cancel_Button, Short_String, ECSOCS_Check, Short_Title, Players_String, NoSound_Check, Developer_String, Folder_String
  ...
  If OpenWindow(#EDIT_WINDOW, 0, 0, 424, 500, "Edit Database", #PB_Window_SystemMenu | #PB_Window_WindowCentered,WindowID(#MAIN_WINDOW))
    ...
    TextGadget(#PB_Any, 0, 138, 80, 20, "Default Icon", #PB_Text_Center)
    icon_string = ComboBoxGadget(#PB_Any, 80, 136, 250, 24)
    icon_button = ButtonGadget(#PB_Any, 340, 136, 76, 24, "Choose") 
    ForEach UM_Database()\UM_Icons()
      AddGadgetItem(icon_string,-1,UM_Database()\UM_Icons())
      If UM_Database()\UM_Icons()=UM_Database()\UM_Default_Icon : count=ListIndex(UM_Database()\UM_Icons()) : EndIf
    Next
    SetGadgetState(icon_string,count)
      ...
      Select event
        ...
        Case #PB_Event_Gadget
          Select EventGadget()
            ...
            Case icon_string
              If EventType()=#PB_EventType_Change
                old_path=Game_Data_Path+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension)
            ...
            Case icon_button
                old_path=Game_Data_Path+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension)
                Choose_Icon()
                ClearGadgetItems(icon_string)
                AddGadgetItem(icon_string,-1,UM_Database()\UM_Default_Icon)
                SetGadgetState(icon_string,0)
                path=Game_Data_Path+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetFilePart(UM_Database()\UM_Default_Icon,#PB_FileSystem_NoExtension)
                RenameFile(old_path+"_Screen.png",path+"_Screen.png")
                RenameFile(old_path+"_Title.png",path+"_Title.png")
                RenameFile(old_path+"_Cover.png",path+"_Cover.png")
    ...
    If change 
      If Short_Names
        SetGadgetItemText(#MAIN_LIST,old_num,UM_Database()\UM_Short+Tags_Short())
      Else
        SetGadgetItemText(#MAIN_LIST,old_num,UM_Database()\UM_Title+Tags())
      EndIf
      Save_DB()
    EndIf
...
Procedure Create_Menus()
  ...
  MenuItem(#MenuItem_16, "Compress Folder")
  MenuBar()
  MenuItem(#MenuItem_17, "Make Bezel")
  MenuBar()
  MenuItem(#MenuItem_18, "Make AGS Folder")
  MenuBar()
  MenuItem(#MenuItem_19, "Scan All Icons")
  MenuItem(#MenuItem_20, "Scan Single Icon")
  MenuTitle("Database")
  OpenSubMenu("Update")
...
Procedure Filter_Window()
  ...
  If OpenWindow(#FILTER_WINDOW, WindowX(#MAIN_WINDOW)-330, WindowY(#MAIN_WINDOW)+30, 325, 650, "Filter", #PB_Window_Invisible)
...
Procedure Draw_Main_Panel(offset.i)
  ...
  ;SetGadgetAttribute(#MAIN_LIST,#PB_ListIcon_DisplayMode,#PB_ListIcon_LargeIcon)
      ...
      Select EventMenu()
        ...
        Case 900 To 1100
          If GetExtensionPart(GetMenuItemText(#POPUP_MENU,EventMenu()))="txt"
            Text_Viewer(Game_Data_Path+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetMenuItemText(#POPUP_MENU,EventMenu()))
          EndIf
          
          If GetExtensionPart(GetMenuItemText(#POPUP_MENU,EventMenu()))="pdf"
            PDF_Viewer(Game_Data_Path+UM_Database()\UM_Type+"\"+UM_Database()\UM_Subfolder+"\"+UM_Database()\UM_Folder+"\"+GetMenuItemText(#POPUP_MENU,EventMenu()))
          EndIf
...
;               EndIf 
        ...
        Case #MenuItem_15 ;{- Re-install game
          ...
          ;}
        Case #MenuItem_17 ;{- Make Bezel
          Create_Bezel(3)
          ;}
        Case #MenuItem_18 ;{- Make AGS Folder
          Create_AGS_Files()
          ;}
        Case #MenuItem_19 ;{- Scan All Icon Files
          Scan_Icons()
          ;}
        Case #MenuItem_20 ;{- Scan Icon File
          Scan_Single_Icon()
          ;}
          ...
          If EventDropFiles()>""
            If GetGadgetState(#MAIN_LIST)<>-1              
              If GetExtensionPart(EventDropFiles())="png" Or GetExtensionPart(EventDropFiles())="jpg" Or GetExtensionPart(EventDropFiles())="iff"
                ...
                EndIf
                If GetGadgetState(#MAIN_PANEL)=1
                  path=Game_Data_Path+"CD32\"+CD32_Database()\CD_Title+"_Cover.png"
                  Resize_Cover(EventDropFiles(), path)
                  Draw_CD32_Info(CD32_Numbers())
                EndIf  
              Else
          ...
          If EventDropFiles()>""
            If GetGadgetState(#MAIN_LIST)<>-1
              If GetExtensionPart(EventDropFiles())="png" Or GetExtensionPart(EventDropFiles())="jpg" Or GetExtensionPart(EventDropFiles())="iff"
                ...
                EndIf
                If GetGadgetState(#MAIN_PANEL)=1
                  path=Game_Data_Path+"CD32\"+CD32_Database()\CD_Title+"_Title.png"
                  Resize_Image(EventDropFiles(), path)
                  Draw_CD32_Info(CD32_Numbers())
                EndIf 
              Else
          ...
          If EventDropFiles()>""
            If GetGadgetState(#MAIN_LIST)<>-1
              If GetExtensionPart(EventDropFiles())="png" Or GetExtensionPart(EventDropFiles())="jpg" Or GetExtensionPart(EventDropFiles())="iff"
                ...
                EndIf
                If GetGadgetState(#MAIN_PANEL)=1
                  path=Game_Data_Path+"CD32\"+CD32_Database()\CD_Title+"_Screen.png"
                  Resize_Image(EventDropFiles(), path)
                  Draw_CD32_Info(CD32_Numbers())
                EndIf 
              Else
...
;             EndIf
    ...
    Case #PB_Event_SizeWindow
      If EventWindow()=#PDF_WINDOW
        ResizeGadget(#PDF_WEBGADGET,0,0,WindowWidth(#PDF_WINDOW),WindowHeight(#PDF_WINDOW))
      EndIf
    ...
    Case #PB_Event_CloseWindow
      ...
      EndIf
      If EventWindow()=#TEXTVIEW_WINDOW
        CloseWindow(#TEXTVIEW_WINDOW)
      EndIf 
      If EventWindow()=#PDF_WINDOW
        CloseWindow(#PDF_WINDOW)
      EndIf
  EndSelect
