    ...
    While NextDirectoryEntry(Folder_LIST)
      Select DirectoryEntryType(Folder_LIST)
        Case #PB_DirectoryEntry_File
          ...
          Files()\R_File_SHA1 = FileFingerprint(Files()\R_File_Name,#PB_Cipher_SHA1)
        Case #PB_DirectoryEntry_Directory
          Select DirectoryEntryName(Folder_LIST)
            Case ".", ".."
              Continue
...
Procedure Draw_Game_Info(*nul)
  ...
  If GetGadgetState(#MAIN_LIST)<>-1
    ...
    If UM_Database()\UM_SPS<>0 : Add_Output(LSet("SPS",11," ")+": "+Str(UM_Database()\UM_SPS)+#CRLF$) : EndIf
    Add_Output(#CRLF$)
...
Procedure Save_GL_CSV(path.s,type.i,A500.b)
...
Procedure Update_FTP()
  ...
  If ListSize(Delete_List())>0
    ...
    ForEach Delete_List()
      ...
      result=DeleteFile(GetCurrentDirectory()+Delete_List())
      Delay(100)
    Next
    PrintS()
...
Macro Make_AGS_Image(image_path)
  ...
  path="-quiet -overwrite -out iff -o "+new_file$+" -c 1 -rtype lanczos -ratio -resize 320 256 -canvas 320 256 center -bgcolor 0 0 0 -resize 320 256 -floyd -colors 96 "+source_file$
  ...
  path="-quiet -overwrite -out iff -o "+new_file$+" -c 1 -resize 320 256 -rtype lanczos -floyd -colors 64 "+source_file$
  If FileSize(GetCurrentDirectory()+RemoveString(new_file$,#DOUBLEQUOTE$))=-1 Or force_iff=#True
    RunProgram(nconvert_path,path,"",#PB_Program_Wait)
  EndIf
...
Macro Make_AGS_Run(info_path)
  ...
  Add_Script("echo "+#DOUBLEQUOTE$+"| *E[32mAmiga Game Selector v2.0 is completely free. If you *E[31m |"+#DOUBLEQUOTE$+#LF$)
  ...
  Add_Script("echo "+#DOUBLEQUOTE$+"| *E[32m       Developed by Paul Vince - Copyright 2023     *E[31m |"+#DOUBLEQUOTE$+#LF$)
  ...
  Add_Script("   Copy "+#DOUBLEQUOTE$+"AGS:"+list_start+current_folder+info_path+".run"+#DOUBLEQUOTE$+" To "+#DOUBLEQUOTE$+"AGS:+  Favourites.ags/"+info_path+".run"+#DOUBLEQUOTE$+#LF$)
  Add_Script("   Copy "+#DOUBLEQUOTE$+"AGS:"+list_start+current_folder+info_path+".txt"+#DOUBLEQUOTE$+" TO "+#DOUBLEQUOTE$+"AGS:+  Favourites.ags/"+info_path+".txt"+#DOUBLEQUOTE$+#LF$)
  Add_Script("   Copy "+#DOUBLEQUOTE$+"AGS:"+list_start+current_folder+info_path+"-1.iff"+#DOUBLEQUOTE$+" TO "+#DOUBLEQUOTE$+"AGS:+  Favourites.ags/"+info_path+"-1.iff"+#DOUBLEQUOTE$+#LF$)
  Add_Script("   Copy "+#DOUBLEQUOTE$+"AGS:"+list_start+current_folder+info_path+"-2.iff"+#DOUBLEQUOTE$+" TO "+#DOUBLEQUOTE$+"AGS:+  Favourites.ags/"+info_path+"-2.iff"+#DOUBLEQUOTE$+#LF$)
  Add_Script("   Copy "+#DOUBLEQUOTE$+"AGS:"+list_start+current_folder+info_path+"-3.iff"+#DOUBLEQUOTE$+" TO "+#DOUBLEQUOTE$+"AGS:+  Favourites.ags/"+info_path+"-3.iff"+#DOUBLEQUOTE$+#LF$)
  Add_Script("   Echo "+#DOUBLEQUOTE$+#DOUBLEQUOTE$+#LF$)
  ...
  If UM_Database()\UM_NoCycle
    Add_Script(" Ex Scripts:NoCycle"+#LF$)
  EndIf
...
Procedure Create_AGS_Files()
  ...
  Protected file, script.s, commands.s, commands_jit.s, source_folder$, source_file$, new_file$, dest_folder$, old_pos.i, slave.s ; short_name.s
  Protected out_folder.s, key$, force_iff.b, set_type.i, add_restricted.b
  ...
  If AGS_Path<>""
    ...
    Save_GL_CSV(AGS_Path+"gameslist.csv",0,#False)
    Save_GL_CSV(AGS_Path+"gameslist_noadult.csv",1,#False)
    Save_GL_CSV(AGS_Path+"gameslist_games.csv",2,#False)
    Save_GL_CSV(AGS_Path+"gameslist_gamesnoadult.csv",3,#False)
    Save_GL_CSV(AGS_Path+"gameslist_demos.csv",4,#False)
    Save_GL_CSV(AGS_Path+"gameslist_demosnoadult.csv",5,#False)
        Save_GL_CSV(AGS_Path+"gameslist_A500.csv",0,#True)
    Save_GL_CSV(AGS_Path+"gameslist_noadult_A500.csv",1,#True)
    Save_GL_CSV(AGS_Path+"gameslist_games_A500.csv",2,#True)
    Save_GL_CSV(AGS_Path+"gameslist_gamesnoadult_A500.csv",3,#True)
    Save_GL_CSV(AGS_Path+"gameslist_demos_A500.csv",4,#True)
    Save_GL_CSV(AGS_Path+"gameslist_demosnoadult_A500.csv",5,#True)
...
Procedure Edit_GL(number) 
  ...
  Protected Next_Button, Prev_Button, Publisher_Text, Alt_Name_String, Alt_Name_Short_String, WHD_Button, Restricted_Check, Data_Disk
  ...
  If OpenWindow(#EDIT_WINDOW, 0, 0, 424, 560, "Edit Database ("+Str(ListIndex(List_Numbers()))+" of "+Str(ListSize(List_Numbers()))+")", #PB_Window_SystemMenu | #PB_Window_WindowCentered, WindowID(#MAIN_WINDOW))
    ...
    PD_Check = CheckBoxGadget(#PB_Any, 162, 336, 50, 20, "PD")
    ...
    TextGadget(#PB_Any, 8, 378, 46, 20, "Type")
    Type_Combo = ComboBoxGadget(#PB_Any, 54, 376, 88, 24)
    TextGadget(#PB_Any, 158, 378, 50, 20, "Config")  
    Config_Combo = ComboBoxGadget(#PB_Any, 212, 376, 124, 24)
    TextGadget(#PB_Any, 8, 410, 60, 20, "Genre")
    Genre_Combo = ComboBoxGadget(#PB_Any, 54, 408, 216, 24) 
    TextGadget(#PB_Any, 8, 440, 60, 20, "Folder")   
    Folder_String = StringGadget(#PB_Any,54,438,336,24,UM_Database()\UM_Folder)
    TextGadget(#PB_Any, 8, 470, 40, 20, "HOL #")   
    HOL_String = StringGadget(#PB_Any,50,468,50,24,UM_Database()\UM_HOL_Entry)
    TextGadget(#PB_Any, 110, 470, 40, 20, "Lemon #")    
    Lemon_String = StringGadget(#PB_Any,154,468,50,24,UM_Database()\UM_Lemon_Entry)
    TextGadget(#PB_Any, 212, 470, 60, 20, "WHDLoad") 
...
EndDataSection
; IDE Options = PureBasic 6.02 beta 1 LTS (Windows - x64)
; CursorPosition = 3508
; FirstLine = 1873
; Folding = BAgBAIQAYAYBAA1BifChCYAAAAAAAAAg
; Optimizer
; EnableThread
; EnableXP
; DPIAware
